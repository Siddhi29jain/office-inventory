...
export default function App() {
  ...
  const addItem = async (e) => {
    ...
    const newItem = { name: formData.get('name'), category: formData.get('category'), quantity: parseInt(formData.get('quantity')), unit: formData.get('unit'), minThreshold: parseInt(formData.get('minThreshold')) };
                      ...
                      <tr key={item.id} className="hover:bg-slate-50">
                        ...
                        <td className="p-4 text-center">
                          <div className="inline-flex bg-slate-100 rounded-lg p-1">
                            <button onClick={() => updateQuantity(item.id, -1)} className="p-1 hover:bg-white rounded"><Minus className="w-4 h-4" /></button>
